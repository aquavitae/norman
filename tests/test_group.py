...
class TestAPI:
    ...
    def test_meta_iter(self):
        r = set(iter(self.g))
        assert r == {self.r[0], self.r[2], self.r[4]}
    ...
    def test_get_noargs(self):
        assert self.g.get() == {self.r[0], self.r[2], self.r[4]}

    def test_get_args(self):
        assert self.g.get(oid=0) == {self.r[0]}

    def test_iter_noargs(self):
        assert set(self.g.iter()) == {self.r[0], self.r[2], self.r[4]}

    def test_iter_args(self):
        assert set(self.g.iter(oid=0)) == {self.r[0]}

    def test_delete_noargs(self):
        self.g.delete()
        assert set(T) == {self.r[1], self.r[3], self.r[5], self.r[6]}
